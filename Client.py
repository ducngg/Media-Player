...
class Client:
    ...
    def __init__(self, master, serveraddr, serverport, rtpport, filename):
        ...
        Default:  
        -> {INIT}
            -> press PLAY -> call setupMovie()
        -> {READY} -> listenRTP() 
        -> {PLAYING}
            -> press PAUSE -> PAUSE
            -> press X     -> PAUSE, if choose ok then QUIT
    ...
    def createWidgets(self):
        ...
        self.describe = Button(
            self.master, 
            width=15, padx=3, pady=3,
            text="ⓘ", bg="white", fg="black", font=('Helvetica', 14, 'bold'),
            command=self.describeMovie,
            state='disabled'    # Normal when playing
        )
        self.describe.grid(row=2, column=3, padx=2, pady=2)

        # Label for displaying the movie
        self.label = Label(
            self.master, 
            height=18, 
            bg= "#A5D2EB"
        ...
        # Label for displaying total time of the movie
        self.totaltimeBox = Label(
            self.master, 
            width=16, 
            text="Total time: 00:00", bg= "#A5D2EB"
        )
        self.totaltimeBox.grid(row=1, column=3, columnspan=1, padx=5, pady=5)

        # Label for displaying remaining time of the movie
        self.remainTimeBox = Label(
            self.master, 
            width=16, 
            text="Remaining time: 00:00", bg="#A5D2EB"
        )
    ...
    def displayDescription(self, lines):
        ...
        Lb1.insert(2, "Name video file: " + str(self.fileName))
        ...
        Lb1.insert(11, "Current playing: " + "%02d:%02d" % (self.currentTime // 60, self.currentTime % 60))
        Lb1.pack()
